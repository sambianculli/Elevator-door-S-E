...
void setup() {
  ...
  // ===============================
  // MANUAL ENABLE (IMPORTANT)
  // ===============================
  pinMode(8, OUTPUT);
  digitalWrite(8, LOW);   // TRY HIGH if motor still doesn't move
...
void limitPins() {
  ...
  int32_t target = stepper->getPositionAfterCommandsCompleted();

  if ((digitalRead(openStopLimit) == LOW) && (target > pos)) {
    stepper->forceStop();
    stepper->setCurrentPosition(fullTravelSteps);
  }

  if ((digitalRead(closeStopLimit) == LOW) && (target < pos)) {
    stepper->forceStop();
    stepper->setCurrentPosition(0);
  }
}
